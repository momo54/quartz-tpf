...
QUERIES=$1
SERVERS='http://52.39.116.115/watDiv_100 http://52.33.245.25/watDiv_100 http://54.70.48.92/watDiv_100'
...
while IFS='' read -r line || [[ -n "$line" ]]; do
  ...
  echo $line > $QFILE
  bin/tpf-client.js model $SERVERS -f $QFILE -o models-3/query$cpt.json
  rm -f $QFILE
  cpt=$((cpt+1))
done < "$QUERIES"
